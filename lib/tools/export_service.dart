...
class ExportService {
  ...
  Future<List<ImageModel>> _fetchAllImages() async {
    ...
    do {
      ...
      final uri = Uri.parse('${UserSession().baseUrl}/api/image?page=$currentPage&pageSize=30&category=$category');
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          allImages.addAll(imageData.map((img) => ImageModel.fromJson(img)).toList());
  ...
  Future<void> exportImages() async {
    try {
      ...
      for (final category in categoryGroups.keys) {
        ...
        // 按collectorType分组
        final typeGroups = groupBy(categoryGroups[category]!, (image) => image.collectorType);
          ...
          for (final image in typeGroups[collectorType]!) {
            ...
            try {
              ...
              if (image.questions != null && image.questions!.isNotEmpty) {
                ...
                configData.add(_buildConfigItem(
                  image, 
                  question,
                  path.join(dateFolder, category, collectorType, path.basename(imageFile.path))
                ));
              }
            } catch (e) {
  ...
  // 构建配置文件条目
  Map<String, dynamic> _buildConfigItem(ImageModel image, QuestionModel question, String imagePath) {
    // 生成选项文本 (A, B, C...)
    String optionsText = '';
    ...
    return {
      "image": imagePath,
      "text_md5": image.fileName!.replaceAll(path.extension(image.fileName!), ''),
